  ...
  steps: [
    {
      ...
      description:
        "The DeepSeek V4 technical report, “DeepSeek-V4: Towards Highly Efficient Million-Token Context Intelligence,” was submitted to arXiv on April 26, 2026, as paper 2606.19348[1]. It is a preview paper that documents the two MoE models DeepSeek released two days earlier on April 24: [[v4-pro|DeepSeek V4 Pro]] (1.6T total / 49B active parameters) and [[deepseek-v4-flash|DeepSeek V4 Flash]] (284B total / 13B active), both supporting a 1M-token context[1].",
      paragraphs: [
    ...
    },
    {
      ...
      list: [
        ...
        "mHC (Manifold-Constrained Hyper-Connections): residual mappings are constrained to the doubly stochastic matrix manifold (the Birkhoff polytope), with spectral norm at most 1, to stabilize cross-layer signal propagation[2].",
        "Muon: replaces AdamW as the optimizer for most modules, delivering faster convergence and more stable training[2].",
      ],
      paragraphs: [
        "Against V3, the report lists further changes: attention moved from the MLA family to the CSA/HCA hybrid, several early dense FFN layers became Hash-routing MoE, expert affinity activation switched from Sigmoid to Sqrt(Softplus(·)), and QK-Clip was removed in favor of Muon plus RMSNorm-normalized attention logits[2].",
      ],
    ...
    },
    {
      ...
      description:
        "The efficiency numbers are the heart of the paper. In the 1M-token setting, V4-Pro needs only 27% of V3.2's single-token inference FLOPs and 10% of its KV cache[1]. V4-Flash does even better: 10% of the FLOPs and 7% of the KV cache[2].",
      paragraphs: [
        "Against a BF16 GQA-8 (head dim 128) baseline, the report puts the V4 family's KV cache at 1M context at roughly 2% of that baseline[2]. That is what makes 1M context a practical default instead of a research demo.",
        "Where the savings come from: KV hybrid storage that keeps RoPE dimensions in BF16 and everything else in FP8 (roughly half the KV memory of a pure-BF16 layout), FP4 compute inside the lightning indexer, smaller attention top-k than V3.2, and the compressed attention itself[2].",
        "One precision note: routed expert parameters use FP4 while most other parameters stay FP8. FP4×FP8 GEMMs already match the peak FLOPs of FP8×FP8 on current hardware, and the paper argues future hardware could be about one-third more efficient[2].",
      ],
    ...
    },
    {
      ...
      description:
        "DeepSeek pretrained V4-Flash on 32T tokens and V4-Pro on 33T tokens. The corpus expands multilingual data to cover cross-cultural long-tail knowledge, with special emphasis on long-document sources such as scientific papers and technical reports[2].",
      paragraphs: [
        "Post-training follows a two-stage paradigm. Stage one cultivates a dedicated domain expert for each target domain — math, code, agents, instruction following — using SFT followed by GRPO reinforcement learning. Stage two merges the experts into one unified model through on-policy distillation, where a student model learns from multiple teachers using a reverse KL loss[2][3].",
      ],
      note: "The abstract summarizes pretraining as “more than 32T” tokens; the report body gives 32T for Flash and 33T for Pro[2].",
    },
    {
      num: "05",
      title: "Official Base Model Benchmarks",
      description:
        "The V4-Pro model card publishes a Base Model benchmark table comparing V3.2-Base, V4-Flash-Base, and V4-Pro-Base on identical rows[3]. It is the cleanest way to see what the architecture buys before any post-training.",
      table: {
      ...
      paragraphs: [
        "Read it this way: V4-Pro-Base crushes V3.2-Base on world knowledge (Simple-QA 55.2 vs 28.3, FACTS Parametric 62.6 vs 27.1) and on long context (LongBench-V2 51.5 vs 40.2)[3]. V4-Flash-Base, with only 13B active parameters, beats V3.2-Base's 37B active on most rows[3].",
    ...
    },
    {
      ...
      paragraphs: [
        "DeepSeek has announced peak/off-peak pricing — 2x during peak hours, defined as 9:00–12:00 and 14:00–18:00 Beijing time — but it is not yet in effect as of August 1, 2026[7]. For the full cost math, including the ~98% context-cache discount, see the [[flash-pricing|V4 Flash pricing guide]].",
        "The report and model cards also publish recommended sampling. For V4-Pro local deployment: temperature 1.0, top_p 1.0, and at least 384K of context for Think Max mode[3]. For Flash-0731: temperature 1.0, top_p 0.95 in agent scenarios and 1.0 otherwise; high/max effort suggests a 384K max output[4]. A minimal call looks like this:",
      ],
...
# Chain of thought arrives in reasoning_content; the answer is in content.`,
      note: "Pricing rows come from DeepSeek's official pricing page, captured August 1, 2026[7]. Prices, concurrency limits, and the peak-pricing rollout date can change.",
    },
    {
      num: "07",
      title: "Local Deployment Commands",
      description:
        "Both model cards ship official serving commands. This is the Flash-0731 vLLM command from the model card, built for a single node with 4×GB300 GPUs and DSpark speculative decoding enabled[4]:",
  ...
  --speculative-config '{"method":"dspark","num_speculative_tokens":7,"draft_sample_method":"greedy"}'`,
      paragraphs: [
        "DSpark is DeepSeek's speculative-decoding module, bundled in the 0731 release. The Hugging Face UI shows 304B parameters for the 0731 repo precisely because the DSpark draft module is included — the base model itself is still 284B[4].",
        "Two deployment notes. The 0731 repo has no Jinja chat template; the official encoding/ directory provides encode_messages and parse_message_from_completion_text instead[4]. And for the full local workflow — weights, VRAM, and quantized options — read the [[flash-huggingface|HuggingFace download and local run guide]].",
      ],
    ...
    },
    {
      ...
      description:
        "The report explains the architecture; the model cards explain how to use it. The key to reading either V4 model card is the mode comparison, because every benchmark is reported per reasoning mode[3].",
      paragraphs: [
        "DeepSeek defines three reasoning modes: Non-think for fast, intuitive everyday tasks; Think High with explicit <think> reasoning for complex problems and planning; and Think Max with a dedicated system prompt that pushes the model toward the boundary of its reasoning ability[3]. The API maps these onto reasoning_effort values of low / high / max, and thinking is on by default[8].",
        "Mode choice moves scores. On the Pro card, SWE-bench Verified goes from 73.6 in Non-Think to 80.6 in Max mode; SimpleQA jumps from 45.0 to 57.9; LiveCodeBench reaches 93.5 in Max; Codeforces hits 3206[3]. When comparing DeepSeek against another model, make sure both ran at the same effort setting.",
      ],
      table: {
      ...
      list: [
        "Separately, the Flash-0731 card reports nine agent benchmarks where the re-post-trained Flash beats the V4-Pro preview across the board — Terminal Bench 2.1 at 82.7 vs 72.1, DeepSWE at 54.4 vs 12.8, Cybergym at 76.7 vs 52.7[4].",
        ...
        "Bottom line: treat every number in the DeepSeek V4 technical report and the model cards as vendor-reported until an independent harness exists. The architecture claims, the efficiency ratios, and the two-stage post-training recipe are all DeepSeek's own statements[2][3].",
      ],
