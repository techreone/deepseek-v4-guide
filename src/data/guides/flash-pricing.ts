...
export const flashPricing: GuideContent = {
  ...
  steps: [
    {
      ...
      description:
        "The official DeepSeek V4 Flash pricing page lists three numbers per 1M tokens: $0.14 for input on a cache miss, $0.0028 for input on a cache hit, and $0.28 for output.[1] All V4 models ship with a 1M-token native context at no extra charge. Billing is simple: expense = number of tokens x price, with no monthly fee and no per-seat charge.",
      paragraphs: [
        "DeepSeek's official pricing page is the source of truth, and it carries a standard caveat: \"Product prices may vary and DeepSeek reserves the right to adjust them.\"[1] Third-party trackers re-verified these numbers against the official page on 2026-07-25 and 2026-07-29, and they match.[7]",
      ...
      list: [
        ...
        "The cache hit / cache miss split applies to input only; output has no cache tier.[1]",
        "The old aliases deepseek-chat and deepseek-reasoner were permanently retired on 2026-07-24 15:59 UTC;[3] new integrations call [[deepseek-v4-flash|deepseek-v4-flash]] directly.",
      ],
      table: {
  ...
  -d '{
    ...
    },
    {
      ...
      description:
        "DeepSeek's context caching is automatic. Since 2024-08-02, a disk-based prefix cache stores repeated prompt prefixes, and any token served from that cache is billed at the cache-hit price instead of the cache-miss price.[1] No SDK changes, no headers, no switch to flip — reuse a system prompt, a long document, or few-shot prefixes and the discount applies on its own.",
      paragraphs: [
        "The headline number: $0.0028 versus $0.14 is a 98% discount on input tokens.[1] The rule of thumb for maximizing hits is simple: put the reusable prefix first. System prompts and context documents go at the top, and volatile content goes at the bottom. At a 90% hit rate, the effective input price drops from $0.14 to about $0.017 per 1M tokens, a reduction of roughly 88%.",
        "A worked example: 1M input tokens on a full cache miss plus 1M output tokens costs $0.14 + $0.28 = $0.42.[1] With a stable system prompt driving a 90% hit rate, the same call lands around $0.30.",
      ],
      list: [
        "Cache is best-effort — DeepSeek does not guarantee a hit rate.[1]",
    ...
    },
    {
      ...
      description:
        "DeepSeek prices thinking and non-thinking modes identically — one price row per model, no surcharge for reasoning. But thinking mode generates extra reasoning tokens, and those tokens are billed at the output price of $0.28 per 1M.[1] Enable thinking on a long agentic run and the added reasoning spend is real, so treat it as an output-cost decision, not a free feature.",
      paragraphs: [
        "Thinking is on by default, and the default effort is high.[2] For complex agent requests such as [[flash-ide|Claude Code]] or [[flash-opencode|OpenCode]], the model is automatically set to max. For compatibility, low and medium map to high, and xhigh maps to max.",
      ...
      list: [
        "Reasoning tokens count in output_tokens, so they consume the output price ($0.28 / 1M) on top of the final answer.[1]",
        "Thinking mode also counts toward your output length, which matters against the 384K max-output cap.[2]",
    ...
    },
    {
      ...
      description:
        "Flash runs about a third the price of [[v4-pro|Pro]] per token: $0.14 versus $0.435 on input, and $0.28 versus $0.87 on output.[1] Since both models read and write the same 1M context, the 3x gap is pure margin for workloads that only need Flash-level quality. The common pattern is hybrid routing: route everything to Flash by default and escalate to Pro only when a request fails or confidence is low.",
      paragraphs: [
        "For mixed workloads, that hybrid pattern is roughly 3x cheaper end-to-end than running Pro everywhere. Flash also carries a 2500-request concurrency limit versus Pro's 500, so it absorbs high fan-out better. Pro's edge is capacity, not price — 1.6T total parameters against Flash's 284B.[2]",
      ...
      list: [
        ...
        "Pro's permanent price cut took effect 2026-05-31: $1.74/$0.0145/$3.48 dropped 75% to $0.435/$0.003625/$0.87.[3]",
      ],
    ...
    },
    {
      ...
      description:
        "Against the major frontier models, DeepSeek V4 Flash is the floor of the price table. Verified against official price pages on 2026-07-25, Flash charges $0.14 input and $0.28 output per 1M tokens, while GPT-5.5 runs $5/$30 and Claude Opus 4.8 runs $5/$25.[6] GLM-5.2 at $1.40/$4.40 is the closest mainstream competitor, and Flash still undercuts it by about 10x on input and 15.7x on output.",
      paragraphs: [
        "The multiples compound fast at scale. Flash output is about 107x cheaper than GPT-5.5 and about 54x cheaper than Claude Sonnet 4.6.[6] On input, Flash is roughly 18x cheaper than GPT-5.4 and 36x cheaper than Claude Opus 4.7. One third-party comparison ran the same coding-agent workload at about $12,000/month on GPT-5.5 versus roughly $252/month on DeepSeek V4 Flash — a 48x gap.[7]",
    ...
    },
    {
      ...
      description:
        "DeepSeek's official pricing page footnote states that peak/off-peak pricing will be adopted, with peak-hour prices set at 2x the regular rate across all billing items.[1] The announced peak windows are daily 9:00-12:00 and 14:00-18:00 Beijing time (UTC+8).[8] As of late July 2026, DeepSeek still charges a single flat price, and the effective date has not been announced.",
      paragraphs: [
      ...
      list: [
        ...
        "Watch api-docs.deepseek.com/quick_start/pricing for the official effective date.[1]",
      ],
    ...
    },
    {
      ...
      list: [
        ...
        "Put reusable prefixes first: system prompts, docs, and few-shot examples at the top, volatile content last. At 90% hit, effective input drops from $0.14 to about $0.017 per 1M (-88%).[1]",
        "Cap output with max_tokens. Flash is verbose — Artificial Analysis consumed 210M output tokens running its Intelligence Index, roughly double the category median.[5]",
        "Check third-party providers: [[flash-openrouter|OpenRouter]] lists Flash at $0.0896/$0.1792 (about 36% off official)[4], DeepInfra from $0.09/$0.18, Requesty at $0.10/$0.20.",
