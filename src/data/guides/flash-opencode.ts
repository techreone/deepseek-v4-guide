  ...
  steps: [
    {
      ...
      paragraphs: [
        "DeepSeek officially announced that the V4 family integrates seamlessly with Claude Code, OpenClaw, and OpenCode, and that V4 already powers DeepSeek's own internal agentic coding workflows. The official guidance is direct: keep base_url, just update model to deepseek-v4-pro or deepseek-v4-flash. Both the OpenAI ChatCompletions format and the Anthropic API format are supported.",
        "DeepSeek V4 Flash is the fast, efficient, economical tier of the V4 family. It is a 284B-total / 13B-active MoE model with a 1M token context window and MIT-licensed weights. Its reasoning closely approaches V4 Pro, and it performs on par with V4 Pro on simple agent tasks. Community users on r/opencode call Flash the undisputed price-performance leader and the closest thing to Opus, measuring roughly 100-150 tokens/s with OpenCode.",
      ],
      list: [
        ...
        "Official pricing of $0.14 input / $0.28 output per 1M tokens keeps high-volume work cheap.",
      ],
    },
    {
      num: "02",
      ...
      list: [
        "OpenCode >= v1.14.24 is required for the built-in DeepSeek provider. The old Go-binary build reports v0.0.x and throws 'agent coder not found'; reinstall with npm install -g opencode-ai.",
        "Create a DeepSeek API key from the API Keys page. You paste it in raw, with no Bearer prefix.",
...
# pick a DeepSeek model, then fine-tune with /models`,
    ...
    },
    {
      ...
      description:
        "Prefer full control? Declare DeepSeek as a custom provider in opencode.json at your project root, or in ~/.config/opencode/ for a global setup that applies everywhere.",
      code: `{
  ...
  "autoCompact": true
      ...
      paragraphs: [
        "This agent-split setup is the recommended pattern: V4 Pro writes real code while V4 Flash handles file search, grep, and title generation. Community cost reports land around 85% Pro / 15% Flash, keeping the expensive model on the work that matters.",
...
opencode run --model opencode-go/deepseek-v4-flash --variant max "your task"   # OpenCode Go channel`,
    ...
    },
    {
      ...
      description:
        "OpenCode Go bundles DeepSeek V4 Flash with a flat monthly credit. Subscribe at opencode.ai/auth, copy the API key, then run /connect and pick OpenCode Go.",
      paragraphs: [
    ...
    },
    {
      ...
      description:
        "If you want to skip API keys entirely, Ollama can launch OpenCode pointed at DeepSeek V4 Flash in a single command.",
