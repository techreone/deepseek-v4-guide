  ...
  steps: [
    {
      ...
      note: "\"DeepSeek V4 official\" means V4-Flash 0731 as of 2026-08-01. V4 Pro remains a preview, with the official changelog stating the release \"will follow soon.\"[7]",
    },
    {
      num: "02",
      title: "Price After the 80% Cut: $0.20/$1.20 vs $0.14/$0.28",
      description:
        "The July 30 cut moved Luna from $1 input / $6 output per 1M tokens to $0.20 / $1.20 — an 80% drop that OpenAI attributes to inference and infrastructure efficiency gains.[1] DeepSeek Flash kept its launch price: $0.14 input, $0.28 output, with cache hits at $0.0028, roughly a 98% discount.[8] The math below is current as of 2026-08-01.",
      paragraphs: [
        "The gap is now almost entirely on the output side. Luna's uncached input is only $0.06 more than Flash's, but its output is more than 4x Flash's. Luna also has a pricing rule Flash does not: any request whose input exceeds 272K tokens is billed at 2x input and 1.5x output prices.[3]",
...
curl https://api.openai.com/v1/responses \\
  ...
  -d '{"model": "gpt-5.6-luna",
    ...
    },
    {
      ...
      paragraphs: [
        "The more striking number is cost per task. Even after OpenAI's 80% cut, Artificial Analysis reports that V4 Flash 0731 on DeepSeek's official API runs about 60% lower cost per task than Luna at max effort — Flash is cheaper per token and finishes tasks at comparable quality.[4]",
        "Same-harness references: GLM-5.2 (max) also scores 51, and open-source leader Kimi K3 (max) scores 57.[4] Luna sits above the 17 median for its price band — a strong model priced near Flash.",
      ],
      list: [
        "Flash 0731 (Reasoning Max): 50 — up from 40 on the preview, and 6 above V4 Pro (44).[4]",
        "Luna (max): 51; Luna (xhigh): 49; Luna (high): 46 — effort level moves the score, so pick it deliberately.[5]",
        "Cost per task is where the models truly diverge: roughly 60% in Flash's favor on DeepSeek's official API.[4]",
      ],
      table: {
    ...
    },
    {
      ...
      description:
        "Both models are strong at agentic coding, and both vendors self-report the headline numbers. DeepSeek reports Flash 0731 at 82.7 on Terminal-Bench 2.1 — up from 61.8 on the preview — while OpenAI reports Luna at 84.7.[7][2] The caveat is real: these come from separate vendors' harnesses, so treat them as directional, not a controlled head-to-head.",
      paragraphs: [
        "Luna's official sheet also publishes its own weakness openly: on OpenAI's MRCR v2 8-needle long-context test at 256K-512K, Luna scores 41.3% versus GPT-5.5's 81.5%.[2] If your workload leans on very long documents, that gap is the single clearest documented red flag in this comparison.",
        "Flash's official sheet shows the re-post-training effect: DeepSWE jumped from 7.3 on the preview to 54.4, Cybergym hit 76.7, and Toolathlon (verified) landed at 70.3.[7] For an independent view, prefer the AA Intelligence Index; our [[flash-benchmarks|DeepSeek V4 Flash Benchmarks]] guide covers all nine numbers.",
      ],
      table: {
    ...
    },
    {
      ...
      description:
        "Latency is where these two feel most different. Artificial Analysis measures Flash's non-thinking time-to-first-token at about 1.24s on DeepSeek's official API, while Luna at max effort spends roughly 121.9 seconds thinking first.[6][5] For a user waiting in chat, that is the difference between instant and \"let me check the logs.\"",
      paragraphs: [
        "Throughput is the reverse. Luna streams output at about 172.1 tokens/s at max effort, versus Flash at roughly 113.6-115.1 tokens/s on DeepSeek's official API.[5] Luna is faster once it starts writing; Flash is faster to start at all.",
        "Two caveats. The 1.24s figure is Flash's non-thinking mode; DeepSeek has not published a thinking-mode TTFT, so pair non-thinking Flash against thinking Luna. Verbosity also shifts perceived speed: Flash used about 206M output tokens on the AA Intelligence Index versus Luna's ~130M — more words per task, not always a win.[4]",
      ],
      list: [
        "TTFT: Flash ~1.24s (non-thinking) vs Luna ~121.9s (thinking, max).[6][5]",
        "Output speed: Luna ~172.1 t/s vs Flash ~113.6-115.1 t/s on official APIs.[5]",
        "Output volume on the same index run: ~206M tokens (Flash) vs ~130M (Luna).[4]",
      ],
    ...
    },
    {
      ...
      list: [
        "High-throughput batch and pure cost — DeepSeek V4 Flash. Classification, summarization, routing, RAG lookups, and light realtime assistants care about cost per token and cost per task. Flash is cheapest on both ($0.14/$0.28 plus a ~98% cache discount).[8]",
        ...
        "Few, high-value, hallucination-sensitive outputs — GPT-5.6 Luna. For a complex single-step code task or a deliverable that will be reviewed, the community pattern is \"plan fast, implement, then verify.\" OpenAI reports Luna's coding agent index at 74.6, above Claude Opus 4.8's 72.5.[2]",
    ...
    },
    {
      ...
      paragraphs: [
        "Flash is verbose and, in practice, more prone to confabulation. Artificial Analysis measured its hallucination rate at 84% on its benchmark — down 12 points from the preview, but still in the range users describe as more hallucination-prone than GPT, Gemini, or Claude.[4] After the 0731 update, community users also reported token usage climbing 4-5x on the same tasks. (Community reports, small samples — not a controlled study.)",
        "Luna is slow at high effort and weak at long context. The ~122s thinking TTFT at max effort is measurable, and community users describe it overthinking simple prompts. Its own published MRCR long-context score of 41.3% is the clearest documented gap.[2] Luna is also closed-source with a knowledge cutoff of 2026-02-16; DeepSeek does not disclose Flash's cutoff.",
