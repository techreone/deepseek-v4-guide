  ...
  steps: [
    {
      ...
      paragraphs: [
        "Anthropic calls the Claude Agent SDK 'a powerful, general-purpose agent harness' and treats multi-window context management as one of its core jobs[2]. The harness is what lets one model work for hours across many context windows without falling apart.",
        "DeepSeek's Harness job posts use the same equation — 'Model + Harness = Agent' — and draw the boundary bluntly: everything other than the model itself belongs to the Harness[7]. The JD lists the engineering surface: LLM API, KV Cache, agent loops, tool use, reasoning, planning, skills, MCP, memory, subagents, and multi-agent coordination[7].",
      ],
...
Source: DeepSeek Harness R&D Engineer JD, May 2026`,
    ...
    },
    {
      ...
      description:
        "Anthropic documented four failure modes that appear when a raw model is pointed at a long task with nothing around it[1][2]. All four are harness problems with harness fixes.",
      paragraphs: [
        "Cross-session amnesia. A long task spans multiple context windows, and every new session starts from zero[2]. Compaction is not enough: even Opus 4.5, given only a high-level one-sentence brief on a multi-window loop, could not produce a production-grade web app. The fix is a two-agent split — an initializer sets up the environment once, and a coding agent makes incremental changes, leaving handoff artifacts like init.sh, claude-progress.txt, git commits, and a feature_list.json with 200+ features marked failing[2].",
        "Context anxiety. As a model nears what it believes is its context limit, it rushes to finish[1]. Compaction preserves continuity but never gives a clean slate; a full context reset does. On Sonnet 4.5 the anxiety was strong enough that reset became necessary; by Opus 4.5 it had mostly disappeared and the authors removed the reset[1].",
        "Self-evaluation bias. Asked to judge its own output, a model 'always confidently praises itself, even when quality is plainly mediocre to a human.' The lever is separation: making an independent evaluator harsher is far easier than making a generator more critical of its own work — a GAN-inspired design[1].",
        "Victory declaration. Agents announce 'done' without checking[2]. Unless told to run end-to-end browser tests, Claude would write code and pass unit tests while the feature silently failed end-to-end. The fix is a checklist that only flips to passing after careful testing, plus real test tools — giving Claude tools like Puppeteer 'dramatically improved performance'[2].",
      ],
      note: "Every fix adds machinery around the model, not inside it. That machinery is the AI agent harness — usually a cheaper lever than waiting for a stronger model[1].",
    },
    {
      num: "03",
      title: "Anthropic's Three-Agent Harness: Planner, Generator, Evaluator",
      description:
        "On March 24, 2026, Anthropic published a harness design for multi-hour application development: three agents in a loop — Planner, Generator, Evaluator[1].",
      paragraphs: [
        "The Planner expands a one-to-four-sentence brief into a full product spec, kept at product context and high-level design so wrong implementation details can't cascade into every sprint[1].",
        "The Generator builds one feature per sprint on a React/Vite/FastAPI/SQLite stack with git version control[1]. The Evaluator uses Playwright MCP to open the running app like a human and check UI, API, and database state against hard thresholds; any failure fails the sprint and returns detailed feedback[1].",
        "Each sprint runs on a contract: Generator and Evaluator negotiate what 'done' means before writing code, communicating through files. One level-editor sprint produced a contract with 27 acceptance criteria[1].",
        "Anthropic is blunt: 'Out of the box, Claude is a poor QA agent.' The first version found real problems and then talked itself out of flagging them, until several rounds of prompt tuning aligned it[1].",
      ],
      table: {
      ...
      note: "Model improvements move the design, not the principle. After Opus 4.6 shipped, Anthropic deleted the sprint structure and ran the Evaluator once at the end: 'the space of interesting harness combinations doesn't shrink as models improve. Instead, it moves'[1].",
    },
    {
      num: "04",
      title: "Solo 20 Minutes vs Harness 6 Hours",
      description:
        "Anthropic built the same 2D game with the same model, Opus 4.5, two ways. Solo with a simple prompt: 20 minutes and $9, core broken. Full harness: 6 hours and $200, playable[1].",
      table: {
        headers: ["Setup", "Time", "Cost", "Result"],
        rows: [
          ["Solo agent", "20 minutes", "$9", "Core broken: entities ignored input, wasted layout, silent wiring failures[1]"],
          ["Full harness", "6 hours", "$200", "16-feature spec, ten sprints, playable game with AI-generation features[1]"],
        ],
      },
      paragraphs: [
        "Anthropic's verdict: more than 20 times the cost, but the output-quality difference was immediate[1]. A second demo — a digital audio workstation on Opus 4.6 with the V2 harness — ran 3 hours 50 minutes at $124.70: Planner $0.46, three build rounds $71.08 / $36.89 / $5.88, three QA rounds $3.24 / $3.09 / $4.06[1].",
        "The generator ran two hours solo without sprint decomposition, and QA still caught stub or display-only gaps every round[1]. Caveat from the source: these are single runs, not repeated experiments — a cost breakdown, not a statistical claim[1].",
      ],
      note: "The cost gap is the headline, but the point is what $200 bought: an agent that verified its own work and finished the task[1].",
    },
    {
      num: "05",
      ...
      paragraphs: [
        "LangChain ran Terminal Bench 2.0 with gpt-5.2-codex held fixed and changed only the harness: system prompt, tools, middleware. The score went from 52.8%, outside the top 30, to 66.5%, inside the top 5: plus 13.7 points on an unchanged model. 'We only changed the harness.'[3]",
      ...
      note: "In every row the model is the same and the harness is the only variable. Across all three, harness changes moved cost and quality more than model choice did[4].",
    },
    {
      num: "06",
      ...
      paragraphs: [
        "The first argument is formal: treat the harness as the controller of a closed-loop dynamical system and the LLM as the stochastic policy it drives — so small harness changes can shift performance more than replacing the model[6].",
        "The second is empirical: published benchmarks, industrial deployments, and variance decomposition show harness-induced variance can exceed model-induced variance — even inverting model rankings[6].",
        "The paper demands a disclosure standard: until the harness specification is disclosed, long-horizon agent leaderboard comparisons should be treated as incomplete and potentially misleading[6].",
      ],
    ...
    },
    {
      ...
      description:
        "DeepSeek's Harness job descriptions are the best official window into the product. They confirm the same formula plus a concrete engineering surface[7].",
      paragraphs: [
        "The JD also says model and harness evolve together, with internal real tasks as the feedback source for both the harness product and model-capability training[7].",
        ...
        "The V4 hardware makes the pitch credible. DeepSeek's Hugging Face post highlights two numbers: at a 1M-token context, V4-Pro's per-token inference FLOPs are 27% of V3.2's, and its KV cache memory is just 10%[8]. A harness built around that cache geometry gets a cost advantage a generic harness cannot copy[8].",
      ],
      list: [
        "JD keywords: KV Cache, MCP, Memory, Subagent, Multi-Agent, Tool Use, Planning[7]",
        "Official: model and harness co-evolve; internal tasks feed both[7]",
      ...
      note: "Co-evolution matters for benchmark reading: DeepSeek tunes the harness and the model together, so official scores measure the pair, not the model alone[7].",
    },
    {
      num: "08",
