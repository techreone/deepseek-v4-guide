  ...
  steps: [
    {
      ...
      description:
        "DeepSeek hosts its open weights in one place: the official HuggingFace organization at huggingface.co/deepseek-ai. For the July 31, 2026 final release, the repository you want is deepseek-ai/DeepSeek-V4-Flash-0731 - the deepseek v4 flash huggingface release that went live under the MIT license with ~167 GB of weights across 48 Safetensors shards.",
      paragraphs: [
        "DeepSeek also maintains a model collection at huggingface.co/collections/deepseek-ai/deepseek-v4. As of August 1, 2026, the collection lists seven models spanning the Flash and Pro lines, including base variants for research and the fused DSpark checkpoints.",
      ],
      list: [
        ...
        "deepseek-ai/DeepSeek-V4-Pro / DeepSeek-V4-Pro-Base - the Pro line, still preview as of August 1, 2026",
        "deepseek-ai/DeepSeek-V4-Pro-DSpark - Pro preview weights plus draft module",
      ],
      note: "Security: DeepSeek has never published V4 weights on GitHub. The only official download channels are huggingface.co/deepseek-ai and the ModelScope mirror (modelscope.cn/models/deepseek-ai/DeepSeek-V4-Flash). Treat any other 'official' tarball or mirror as a phishing risk.",
    },
    {
      num: "02",
    ...
    },
    {
      ...
      description:
        "All 284B weights must be resident in memory even though only 13B activate per token. 'Flash' describes inference cost, not footprint. Plan for roughly 170-175 GB of total VRAM for the full-precision weights plus KV cache.",
      paragraphs: [
        "The realistic reference rig for the official FP4+FP8 weights is a 4x H200 node running vLLM with MoE expert parallelism. Two H200s (282 GB) or two RTX Pro 6000 Blackwell cards (192 GB) also work. A single 24 GB consumer card cannot run the full model.",
        "If you want a smaller footprint, the GGUF route is the practical answer. Unsloth repackages the official quantization-aware-trained weights bit-exact at the expert level, so you do not lose precision where it matters most.",
      ],
      table: {
...
--chat-template-kwargs '{"reasoning_effort":"max"}'  # set reasoning tier`,
    ...
    },
    {
      ...
      description:
        "Downloading 167 GB only makes sense if you have the hardware or a real use case. For most developers, the official API is the pragmatic choice: model id deepseek-v4-flash, input at $0.14 per 1M tokens, output at $0.28 per 1M tokens, and a ~98% cache-hit discount that drops input to $0.0028.",
      paragraphs: [
