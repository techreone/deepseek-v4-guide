  ...
  steps: [
    {
      ...
      description:
        "DeepSeek V4 Flash is a 284B-parameter Mixture-of-Experts (MoE) model that activates only 13B parameters per token. It supports a native 1M-token context and up to 384K tokens of output.",
      paragraphs: [
      ...
      note: "The 0731 official release keeps the same architecture and size as the April preview. It was only re-post-trained.",
    },
    {
      num: "02",
      ...
      paragraphs: [
        "DeepSeek V4 Flash has 284B total parameters, but the router activates roughly 4.6% of them — about 13B — for each token. That active count decides how much compute each token costs. The total decides how much memory you need.",
      ...
      list: [
        ...
        "Routed experts make up about 96% of the model and are native MXFP4.",
    ...
    },
    {
      ...
      description:
        "Full-precision inference of the native FP4/FP8 weights needs roughly 170–175 GB of total VRAM: about 158 GB of weights, ~10 GB for a full 1M-context KV cache, plus runtime overhead.",
      paragraphs: [
        "That fits on 2x H200 (282 GB) or 2x RTX Pro 6000 Blackwell (192 GB). The commonly cited reference deployment is a 4x H200 node with vLLM and MoE expert parallelism. On the Hopper side, the FP8-only weights (~284 GB) require 8x H100 in an HGX chassis with tensor parallelism 8.",
    ...
    },
    {
      ...
      description:
        "NVIDIA publishes an NVFP4 variant, nvidia/DeepSeek-V4-Flash-NVFP4, that requantizes the MoE experts to standard NVFP4 while attention, shared experts, router head, and MTP stay in FP8. It is quantized with the NVIDIA Model Optimizer and requires a Blackwell GPU.",
      paragraphs: [
  ...
  --speculative-config '{"method":"mtp","num_speculative_tokens":3}'`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "For a 24–48 GB consumer card, the practical recommendation is to use the DeepSeek API ($0.14 per 1M input, $0.28 per 1M output tokens) or a smaller distilled model. Running V4 Flash locally is a workstation or server-class deployment.",
      ],
      table: {
